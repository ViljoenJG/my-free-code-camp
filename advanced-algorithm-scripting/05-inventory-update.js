...
function updateInventory(arr1, arr2) {
    ...
    var currInvItemNames = arr1.map(a => a[1]);
    var curInvItemCnt = arr1.map(a => a[0]);
    var updatedInv = [];

    for (var a=0; a<arr2.length; a++) {
      var newItem = arr2[a];
      var invIdx = currInvItemNames.indexOf(newItem[1]);

      if (invIdx != -1) {
        curInvItemCnt[invIdx] += newItem[0];
      } else {
        updatedInv.push(newItem);
      }
    }

    return updatedInv.concat(
      currInvItemNames.map(
        (curr, idx) => [curInvItemCnt[idx], curr]
      )
    ).sort(sortByName);
}
